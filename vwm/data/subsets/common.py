...
def read_frame(dataset_path, index):
    dataset = load_from_disk(dataset_path)
    frame_data = dataset[index]['bytes']
    with io.BytesIO(frame_data) as buffer:
...
class BaseDataset(Dataset):
    ...
    def preprocess_image_arrow(self, image_path):
        dataset_path = os.path.join(*image_path.split('/')[:-1])+'.arrow'
        index = int(image_path.split('/')[-1].split('.')[0])
        image = read_frame(dataset_path, index)
        ori_w, ori_h = image.size
        if ori_w / ori_h > self.target_width / self.target_height:
            tmp_w = int(self.target_width / self.target_height * ori_h)
            left = (ori_w - tmp_w) // 2
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.num_frames):
            ...
            image = self.preprocess_image_arrow(img_path)
